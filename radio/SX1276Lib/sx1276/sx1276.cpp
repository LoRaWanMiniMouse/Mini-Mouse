    ...
    case MODEM_LORA:
        {
            ...
            if( ( ( bandwidth == 7 ) && ( ( datarate == 11 ) ||( datarate == 11 ) || ( datarate == 12 ) ) ) ||
               ( ( bandwidth == 8 ) && (( datarate == 11 ) ||( datarate == 12 ) )) )
...
void SX1276::Rx( uint32_t timeout )
{
    ...
    case MODEM_LORA:
        {
            ...
            else
            {
                ...
                // DIO0=RxDone @modify by fabien
                //Write( REG_DIOMAPPING1, ( Read( REG_DIOMAPPING1 ) & RFLR_DIOMAPPING1_DIO0_MASK ) | RFLR_DIOMAPPING1_DIO0_00 );
                Write( REG_DIOMAPPING1, ( Read( REG_DIOMAPPING1 ) & RFLR_DIOMAPPING1_DIO0_MASK & RFLR_DIOMAPPING1_DIO1_MASK  ) | RFLR_DIOMAPPING1_DIO0_00 | RFLR_DIOMAPPING1_DIO1_00 );
    ...
    if( timeout != 0 )
    {
        rxTimeoutTimer.attach_us( this, &SX1276::OnTimeoutIrqtemp, timeout );
...
void SX1276::Tx( uint32_t timeout )
{
 //  DEBUG_PRINT("TX func enter\n");
    ...
    case MODEM_LORA:
        {
             //DEBUG_PRINT("TX func in lora mode enter\n");
            if( this->settings.LoRa.FreqHopOn == true )
            {//DEBUG_PRINT("TX func in lora freqhop enter\n");
                Write( REG_LR_IRQFLAGSMASK, RFLR_IRQFLAGS_RXTIMEOUT |
                                                  RFLR_IRQFLAGS_RXDONE |
                                                  RFLR_IRQFLAGS_PAYLOADCRCERROR |
                                                  RFLR_IRQFLAGS_VALIDHEADER |
        ...
        uint8_t registers[] = { 0x01, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13, 0x014, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27,0x33,0x39};

    uint8_t i;
    for (i = 0; i < sizeof(registers); i++)
    {
   // pcsx.printf("registers[%x]", registers[i]);
    //pcsx.printf(": ");
    //pcsx.printf("%x\n",Read(registers[i]));
    } 
...
//    {
//        rxTimeoutTimer.detach();
//        rxTimeoutTimer.attach_us( this, &SX1276::OnTimeoutIrq, 3e6 );
//        }
//    else 
//     OnTimeoutIrq();   
}
...
void SX1276::OnDio0Irq( void )
{
    ...
    //pcsx.printf("TX interrupt func in  enter irq flag %d\n", Read(REG_LR_IRQFLAGS));
    switch( this->settings.State )
    {                
        case RF_RX_RUNNING:
